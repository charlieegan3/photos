...
func BuildMediaHandler(db *sql.DB, bucket *blob.Bucket) func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		br, err := bucket.NewReader(r.Context(), fmt.Sprintf("thumbs/%d-%s.jpg", medias[0].ID, imageResizeString), nil)
		...
		if gcerrors.Code(err) == gcerrors.NotFound {
			...
			bw, err := bucket.NewWriter(r.Context(), fmt.Sprintf("thumbs/%d-%s.jpg", medias[0].ID, imageResizeString), nil)
			if err != nil {
				w.WriteHeader(http.StatusInternalServerError)
		...
		br, err = bucket.NewReader(r.Context(), fmt.Sprintf("thumbs/%d-%s.%s", medias[0].ID, imageResizeString, medias[0].Kind), nil)
		if err != nil {
			w.WriteHeader(http.StatusInternalServerError)
			w.Write([]byte(err.Error()))
			return
