...
func buildStylesHandler() (handler func(http.ResponseWriter, *http.Request), err error) {
	...
	if err != nil {
		return handler, fmt.Errorf("failed to load normalize css data: %s", err)
	}

	tachyonsData, err := cssContent.ReadFile("static/css/tachyons.min.css")
	if err != nil {
		return handler, fmt.Errorf("failed to load tachyons css data: %s", err)
	}

	siteStyleData, err := cssContent.ReadFile("static/css/styles.css")
	if err != nil {
		return handler, fmt.Errorf("failed to load site styles css data: %s", err)
	}
